...
fn is_palindrome(word: &str) -> bool {
    // todo!()
    let _temp = word.to_string().to_lowercase();
    let _reversed = reverse_string(&_temp);
    if _temp == _reversed {
        return true;
    } else {
        return false;
...
fn count_char_occurrences(string: &str, ch: char) -> usize {
    // todo!()
    let mut _vec_temp = Vec::new();
    let mut s = string.to_string();
    unsafe {
        let _vec = s.as_mut_vec();
        _vec_temp = _vec.to_vec();
    }
    let _result = _vec_temp.iter().filter(|&n| *n == ch as u8).count();
    _result
}
